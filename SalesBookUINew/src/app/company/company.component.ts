...
export class CompanyComponent implements OnInit {
    ...
    getItems() {
        this.getItemSub = this.__CompanyService.getData(this.query)
            .subscribe(data => {
    ...
    openPopUp(data: any = {}, isNew = true) {
        this.__commonService.addeditcompany(data, isNew).subscribe(RtnData => {
            this.getItems();
        })
    }
    ...
    deleteItem(row) {
        if (confirm(`Delete ${row.ProductName}?`)) {
            ...
            // this.loader.open();
            this.__CompanyService.deleteData(row)
